...
static FORCE_INLINE uint32_t getblock32(const uint32_t *p, Py_ssize_t i) {
#if defined(__BYTE_ORDER__) && (__BYTE_ORDER__ == __ORDER_BIG_ENDIAN__)
  return bswap_32(p[i]);
#else
  return p[i];
...
static FORCE_INLINE uint64_t getblock64(const uint64_t *p, Py_ssize_t i) {
#if defined(__BYTE_ORDER__) && (__BYTE_ORDER__ == __ORDER_BIG_ENDIAN__)
  return bswap_64(p[i]);
#else
  return p[i];
...
static FORCE_INLINE uint32_t mixK1(uint32_t k1) {
...
static FORCE_INLINE uint32_t mixH1(uint32_t h1, const uint32_t h2,
                                   const uint8_t shift, const uint32_t c1) {
  h1 = ROTL32(h1, shift);
  h1 += h2;
  h1 = h1 * 5 + c1;
...
static FORCE_INLINE uint64_t mixK_x64_128(uint64_t k1, const uint8_t shift,
                                          const uint64_t c1,
                                          const uint64_t c2) {
  k1 *= c1;
  k1 = ROTL64(k1, shift);
...
static FORCE_INLINE uint64_t mixK1_x64_128(uint64_t k1) {
...
static FORCE_INLINE uint64_t mixK2_x64_128(uint64_t k2) {
  const uint64_t c1 = BIG_CONSTANT(0x87c37b91114253d5);
  const uint64_t c2 = BIG_CONSTANT(0x4cf5ad432745937f);
...
static FORCE_INLINE uint64_t mixH_x64_128(uint64_t h1, uint64_t h2,
                                          const uint8_t shift,
                                          const uint32_t c) {
  h1 = ROTL64(h1, shift);
  h1 += h2;
...
static FORCE_INLINE uint64_t mixK_x86_128(uint32_t k, const uint8_t shift,
                                          const uint32_t c1,
                                          const uint32_t c2) {
  k *= c1;
  k = ROTL32(k, shift);
