...
static void MMH3Hasher32_dealloc(MMH3Hasher32 *self) {
  Py_TYPE(self)->tp_free((PyObject *)self);
}

static PyObject *MMH3Hasher32_new(PyTypeObject *type, PyObject *args,
                                  PyObject *kwds) {
  MMH3Hasher32 *self;
...
static int MMH3Hasher32_init(MMH3Hasher32 *self, PyObject *args,
                             PyObject *kwds) {
...
static PyObject *MMH3Hasher32_update(MMH3Hasher32 *self, PyObject *obj) {
  Py_ssize_t i = 0;
  Py_buffer buf;
  uint32_t h1 = self->h;
  uint32_t k1 = 0;
...
static FORCE_INLINE uint32_t digest32_impl(uint32_t h, uint64_t k1,
                                           Py_ssize_t length) {
  h ^= mixK1(k1);
  h ^= length;
  h = fmix32(h);
...
static PyObject *MMH3Hasher32_digest(MMH3Hasher32 *self,
...
static PyObject *MMH3Hasher32_sintdigest(MMH3Hasher32 *self,
                                         PyObject *Py_UNUSED(ignored)) {
  uint32_t h = digest32_impl(self->h, self->buffer, self->length);
...
static PyObject *MMH3Hasher32_uintdigest(MMH3Hasher32 *self,
                                         PyObject *Py_UNUSED(ignored)) {
  uint32_t h = digest32_impl(self->h, self->buffer, self->length);
  return PyLong_FromUnsignedLong(h);
}

static PyObject *MMH3Hasher32_copy(MMH3Hasher32 *self,
                                   PyObject *Py_UNUSED(ignored)) {
  MMH3Hasher32 *p;
...
static PyObject *MMH3Hasher32_get_digest_size(PyObject *self, void *closure) {
  return PyLong_FromLong(MMH3_32_DIGESTSIZE);
}

static PyObject *MMH3Hasher32_get_block_size(PyObject *self, void *closure) {
  return PyLong_FromLong(MMH3_32_BLOCKSIZE);
}

static PyObject *MMH3Hasher32_get_name(PyObject *self, void *closure) {
...
static void MMH3Hasher128x64_dealloc(MMH3Hasher128x64 *self) {
  Py_TYPE(self)->tp_free((PyObject *)self);
}

static PyObject *MMH3Hasher128x64_new(PyTypeObject *type, PyObject *args,
                                      PyObject *kwds) {
...
static int MMH3Hasher128x64_init(MMH3Hasher128x64 *self, PyObject *args,
                                 PyObject *kwds) {
  static char *kwlist[] = {"seed", NULL};
...
static PyObject *MMH3Hasher128x64_update(MMH3Hasher128x64 *self,
                                         PyObject *obj) {
  Py_ssize_t i = 0;
  Py_buffer buf;
  uint64_t h1 = self->h1;
  ...
  for (; i + 16 <= buf.len; i += 16) {
    ...
    if (self->shift == 0) { // TODO: use bit ops
      ...
      self->buffer1 = k1 >> (128 - self->shift) | (k2 << (self->shift - 64));
...
static PyObject *MMH3Hasher128x64_digest(MMH3Hasher128x64 *self,
                                         PyObject *Py_UNUSED(ignored)) {
...
static PyObject *MMH3Hasher128x64_sintdigest(MMH3Hasher128x64 *self,
                                             PyObject *Py_UNUSED(ignored)) {
  const char out[MMH3_128_DIGESTSIZE];
  digest_x64_128_impl(self->h1, self->h2, self->buffer1, self->buffer2,
                      self->length, out);
...
static PyObject *MMH3Hasher128x64_uintdigest(MMH3Hasher128x64 *self,
                                             PyObject *Py_UNUSED(ignored)) {
  const char out[MMH3_128_DIGESTSIZE];
  digest_x64_128_impl(self->h1, self->h2, self->buffer1, self->buffer2,
                      self->length, out);
...
static PyObject *MMH3Hasher128x64_stupledigest(MMH3Hasher128x64 *self,
                                               PyObject *Py_UNUSED(ignored)) {
  const char out[MMH3_128_DIGESTSIZE];
  digest_x64_128_impl(self->h1, self->h2, self->buffer1, self->buffer2,
                      self->length, out);
...
static PyObject *MMH3Hasher128x64_utupledigest(MMH3Hasher128x64 *self,
                                               PyObject *Py_UNUSED(ignored)) {
  const char out[MMH3_128_DIGESTSIZE];
  digest_x64_128_impl(self->h1, self->h2, self->buffer1, self->buffer2,
                      self->length, out);
...
static PyObject *MMH3Hasher128x64_copy(MMH3Hasher128x64 *self,
                                       PyObject *Py_UNUSED(ignored)) {
  MMH3Hasher128x64 *p;
...
static PyObject *MMH3Hasher128x64_get_digest_size(PyObject *self,
                                                  void *closure) {
  return PyLong_FromLong(MMH3_128_DIGESTSIZE);
}

static PyObject *MMH3Hasher128x64_get_block_size(PyObject *self,
                                                 void *closure) {
  return PyLong_FromLong(MMH3_128_BLOCKSIZE);
}

static PyObject *MMH3Hasher128x64_get_name(PyObject *self, void *closure) {
...
static void MMH3Hasher128x86_dealloc(MMH3Hasher128x86 *self) {
  Py_TYPE(self)->tp_free((PyObject *)self);
}

static PyObject *MMH3Hasher128x86_new(PyTypeObject *type, PyObject *args,
                                      PyObject *kwds) {
...
static int MMH3Hasher128x86_init(MMH3Hasher128x86 *self, PyObject *args,
                                 PyObject *kwds) {
  static char *kwlist[] = {"seed", NULL};
...
static PyObject *MMH3Hasher128x86_update(MMH3Hasher128x86 *self,
                                         PyObject *obj) {
  Py_ssize_t i = 0;
  Py_buffer buf;
  uint32_t h1 = self->h1;
...
static PyObject *MMH3Hasher128x86_digest(MMH3Hasher128x86 *self,
                                         PyObject *Py_UNUSED(ignored)) {
...
static PyObject *MMH3Hasher128x86_uintdigest(MMH3Hasher128x86 *self,
                                             PyObject *Py_UNUSED(ignored)) {
  const char out[MMH3_128_DIGESTSIZE];
  digest_x86_128_impl(self->h1, self->h2, self->h3, self->h4, self->buffer1,
                      self->buffer2, self->buffer3, self->buffer4, self->length,
...
static PyObject *MMH3Hasher128x86_stupledigest(MMH3Hasher128x86 *self,
                                               PyObject *Py_UNUSED(ignored)) {
  const char out[MMH3_128_DIGESTSIZE];
  digest_x86_128_impl(self->h1, self->h2, self->h3, self->h4, self->buffer1,
                      self->buffer2, self->buffer3, self->buffer4, self->length,
...
static PyObject *MMH3Hasher128x86_utupledigest(MMH3Hasher128x86 *self,
                                               PyObject *Py_UNUSED(ignored)) {
  const char out[MMH3_128_DIGESTSIZE];
  digest_x86_128_impl(self->h1, self->h2, self->h3, self->h4, self->buffer1,
                      self->buffer2, self->buffer3, self->buffer4, self->length,
...
static PyObject *MMH3Hasher128x86_copy(MMH3Hasher128x86 *self,
                                       PyObject *Py_UNUSED(ignored)) {
  MMH3Hasher128x86 *p;
...
static PyObject *MMH3Hasher128x86_get_digest_size(PyObject *self,
                                                  void *closure) {
  return PyLong_FromLong(MMH3_128_DIGESTSIZE);
}

static PyObject *MMH3Hasher128x86_get_block_size(PyObject *self,
                                                 void *closure) {
  return PyLong_FromLong(MMH3_128_BLOCKSIZE);
}

static PyObject *MMH3Hasher128x86_get_name(PyObject *self, void *closure) {
